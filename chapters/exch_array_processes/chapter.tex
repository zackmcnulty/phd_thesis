...
Recalling the correspondence \eqref{eqn:K(., alpha) processes characterize h processes} between infinite
 bridges of a Markov chain and its nonnegative harmonic functions via the Doob
 h-transform \eqref{eqn:h-process} discussed in Appendix \ref{appendix:doob_martin_background_appendix}, we see:
...
\noindent An immediate consequence is that the harmonic functions $\tilde{h}$ of the
...
For example, the exchangeable array corresponding to a graphon is disassociated: since edges are added independently,
the random subgraphs corresponding to disjoint collections of vertices are independent in the graphon model.
The following shows these properties characterize the Doob-Martin boundary of
the unlabeled chain.
