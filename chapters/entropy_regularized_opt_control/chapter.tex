\chapter{Diffusion Limits in Entropy-Regularized Stochastic Control}

\noindent \textbf{Goals}
\begin{itemize}
	\item Show convergence of the discrete-time entropy-regularized optimal control problem to the continuous-time exploratory formulation of \cite{wang2020_reinforcement_learning_stochastic_control}.
...
The paper \cite{wang2020_reinforcement_learning_stochastic_control} overcomes this obstacle by defining ``relaxed" or ``averaged" dynamics induced by a policy $\pi$. Namely, let
%
...
\end{equation}
%
\noindent  where $\cA(x)$ is the set of admissible controls, which possibly depends on the state $x$. \cite{wang2020_reinforcement_learning_stochastic_control} call this the \textit{exploratory} formulation, and motivate it as the average local dynamics
across many realizations of the Brownian motion paths. Again, suitable regularity on $b, \sigma$ are required for the SDE \ref{eqn:exploratory_dynamics} to have a unique strong solution.
 ...
 In this short note, we show that the exploratory dynamics of \cite{wang2020_reinforcement_learning_stochastic_control} arise as the scaling limit of
...
stochastic optimal control problem \eqref{eqn:exploratory_optimal_value_function} \red{is well-posed}. As \cite{wang2020_reinforcement_learning_stochastic_control} mention, any $\pi \in \cA(x)$ in the set of admissible
...
%
\noindent As in \cite{wang2020_reinforcement_learning_stochastic_control}, we make the following assumptions.
...
\begin{proof}[Proof of Lemma \ref{lem:convergence_fixed_policy_to_exploratory_dynamics}]
	...
	martingale problem is well-posed. By the standard functional convergence results (e.g. Section 8.7 of \cite{durrett1996_stochastic_calculus_practical_introduction})), we have $Y^h_t \Rightarrow X^\pi_t$
	in $D([0,T], \R^d)$.
\end{proof}
...
this in Section 8.7 of \cite{durrett1996_stochastic_calculus_practical_introduction}, condition (A):
...
to the analogous continuous-time value function defined by the exploratory dynamics under the proper scalings of the relevant parameters. More precisely, we obtain the following result.
%
...
\begin{proof}[Proof of Lemma \ref{lem:convergence_of_value_functions}]
	...
	\begin{proof}[Proof of Claim 1]
		...
		%
		\noindent See for example the proof of Theorem 5.2.1 in \cite{oksendal2003_stochastic_differential_equations}. \zack{The stated proof requires $\sigma$ to be globally Lipschitz rather than just locally; some truncation argument is required maybe.} This gives
		%
...
\noindent A standard localization argument gives the following corollary.
...
%
\noindent which recovers equation 14 in \cite{wang2020_reinforcement_learning_stochastic_control}.
...
\red{We should show the conditions/assumptions we make above naturally hold under the simple case of the LQR.
	Then we should just briefly review the results of \cite{wang2020_reinforcement_learning_stochastic_control} in terms of what the limit looks like, and what our corresponding
