...
\noindent This chapter introduces a symmetry-based approach to estimating the
fair value of an asset from a central limit order book. The bid and ask sides of
the book are encoded as measures, and the fair value is defined as the point
around which these measures are as symmetric as possible. We study this
variational problem under Wasserstein metrics, derive explicit formulas in the
$W_2$ case, incorporate quantity imbalances in a way that recovers the weighted
mid price in a simple limiting case, and prove basic stability properties of the
resulting estimator. We also discuss a rudimentary market-maker model which
connects the symmetry objective to latent pairings between bids and asks.
...
participants are willing to buy (bid) or sell (ask/offer) a specific financialasset at. If there are any bids and asks that are overlapping, they are matched according to some matching rule and removed from the order book. What remains is a collection of bids and asks separated 
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.55\textwidth]{images/ob_figure.png}
...
Many approaches have been proposed for modeling the dynamics of the central limit order book (CLOB), ranging from classical
 point-process and stochastic differential equations to more recent deep-learning and agent-based methods. The paper \cite{jain2024limitorderbooksimulations} 
 provides a recent survey. Although these models differ substantially in methodology, a common goal is to extract useful information from the order 
book, either by estimating the fair value of the underlying asset or by describing how the order book evolves over time. 
In this sense, order book analysis is often less about computing a fundamental value directly 
and more about inferring what other market participants appear to believe the fair value is. Since much of
 the activity in modern markets is driven by large participants with significant informational, technological,
  and experiential advantages, this indirect approach can be more realistic than attempting to estimate fair 
  value in isolation. Of course, such an order-book-based estimate can also be combined with fair-value estimates
   obtained from other sources.



A typical naive model for the fair value is just the \textit{mid price} 
$v_{m} = (b_0 + a_0) / 2$, the average of the best bid and best offer.
...
incorporate further price levels, but they too lack a rigorous motivation.
...
We aim to develop a new notion of the fair value that captures more nuanced 
information about the \textit{shape} of the order book which is optimal in some probabilistic 
sense, and which reduces to the above heuristics under natural assumptions.
 ...
 heuristics while also developing a large class of potential alternatives. 
...
\begin{figure}[h!]
    ...
    reflected ask book and the bid book is measured by some metric $d$. Then $c$ is chosen to minimize this distance.}
...
commonly used heuristics and provides a natural variational characterization of the fair value. When symmetry is measured via the Wasserstein distance, we derive 
...
in \cite{jain2024limitorderbooksimulations}, and compare the resulting
