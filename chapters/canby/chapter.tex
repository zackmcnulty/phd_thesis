...
\noindent More granular bounds are developed in the proof below.
...
survives until $k$. In this case the roles of $J,K$ decouple. By
Lemma~\ref{lem:pop_sizes_in_ID_processes}, the numbers of such $\alpha$ and $\beta$
are at least $\mathrm{Poisson}(m_0(\ell_{ij}))$ and $\mathrm{Poisson}(m_0(\ell_{ik}))$
respectively, where $m_0(t) = \frac{\lambda_N}{\mu}(1-\eta_t)$, and these are
...
\noindent The argument is analogous to the proof of Theorem 2.1 in~\cite{shekhar2018_how_many_genes_are_enough}. In Appendix~\ref{app:anomaly-free_zone}, we briefly discuss the
