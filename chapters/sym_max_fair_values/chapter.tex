...
\cite{jain2024limitorderbooksimulations} for a recent survey. Although these
...
\noindent for any metric $d$ on a suitable space of probability distributions.
 ...
 it implicitly depends on $d$. A visualization of this procedure is given in Figure~\ref{fig:sym-fv-viz}.
...
In the balanced case, a paired-order model takes bids and asks of the form
%
\[\bid = v - h, \qquad \ask = v + h,\]
%
where $h$ is a latent half-spread reflecting uncertainty, inventory concerns, or
compensation for supplying liquidity. With imbalanced buying and selling
pressure, we instead allow asymmetric quotes:
%
\[\bid = v - \theta^{-1}h, \qquad \ask = v + (1-\theta)^{-1}h,\]
%
where $\theta \in (0,1)$ is an exogenous imbalance parameter. As $\theta$
approaches zero, the bid is moved farther from the fair value; as $\theta$
approaches one, the analogous effect occurs on the ask side. Formally, we study:
%
\begin{align*}
...
symmetry of the central limit order book around it. This generalizes several
...
small perturbations of the order book.

Corollary~\ref{corr:w2_fv_is_mean_barycenter} suggests a natural extension: in
the $W_2$ case, the symmetrized fair value is the mean of the weighted
barycenter of $\mu_a$ and $\mu_b$, given by
%
    \[\overline{\mu_\theta} := \argmin_{\nu} \theta d_{W_2}(\nu, \mu_a) + (1-\theta) d_{W_2}(\nu, \mu_b).\]
%
This identifies the fair value with a Wasserstein barycenter and suggests
...
in spirit to the aggregation procedure in \cite{raban2024_aggregate_ratings};
