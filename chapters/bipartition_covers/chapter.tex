...
conditions of the Mixture Lemma (Lemma~\ref{lem:mixtures_and_stochastic_dominance}), which shows first-order stochastic dominance
is preserved by increasing mixtures, where $\{S_{k,m}^T\}_{m=1}^{\lfloor k/2\rfloor}$ are playing the
...
First, replacing every branch length in $\cT$ by $T_{\mathrm{min}}$ can only
...
they arise from disjoint subtrees. We first establish the balanced-subtree split
comparison
\begin{equation}
U_m+U_{k-m}
\leq_{st}
...
The idea of the proof is simple: both $X_m$ and $X_{k-m}$ consist of two subtrees of (approximately)
the same size. By taking one subtree from each of $X_m, X_{k-m}$ and swapping them using the balancing lemma,
we create two balanced subpopulations. From there, we can inductively balance these two subpopulations separately to get the result.
...
\]
By Lemma~\ref{lem:xi_zxi_increasing_in_lro},
\[
...
Hence Corollary~\ref{corollary:stochastic_dominance_composition} yields
\[
U_m+U_{k-m}
