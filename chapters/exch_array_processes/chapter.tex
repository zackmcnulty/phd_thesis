...
provides another potential limit theory. Given a transient Markov chain $\Xn$ on some
...
followed, extending these ideas to hypergraphs~\cite{zhao2015_hypergraph_limits_regularity_approach, elek2012_measure_theoretic_dense_hypergraphs},
...
how several existing examples of exchangeable combinatorial growth processes can be realized in this framework.
...
to extend these embedding densities to the limiting representation functions, and an
embedding of finite arrays into this space of representation functions in a way that
...
We introduce a strong cut norm for which an analog of the classical counting lemma of \cite{lovasz2004_limits_dense_graph_sequences}
holds. However, we show this natural extension generates a topology strictly stronger than the topology of left-convergence.
Furthermore, we discuss how one might possibly extend a suitable metric from binary arrays to arrays with more general state spaces.
...
associated to it: a ``labeled'' chain $L_n$ and an ``unlabeled'' one $M_n$. The finite labeled chain $L_n$ is just the ``top left corner'' of the array: the restriction
...
%
In words, the same permutation is applied consistently to the indices at every level.
...
\label{ex:polyas_urn}
    ...
    %
    \noindent where $(p_1(U),\ldots,p_m(U)) \sim \mathrm{Dirichlet}(\alpha_1,\ldots,\alpha_m)$.
\end{example}
...
\label{example:graphon_chain}
    ...
    constructed via sampling $U_i \sim \operatorname{Unif}[0,1]$ and then connecting vertex $i$ to vertex
     $j$ with probability $w(U_i, U_j)$ \cite{lovasz2004_limits_dense_graph_sequences,borgs2007_convergent_dense_graph_sequences}. Thus a graphon can be represented as an ``infinite adjacency matrix''
     which forms an exchangeable array with entries
    %
    \[X_{ij} = \ind \{U_{ij} \leq w(U_i, U_j)\} \quad \text{for } \{i,j\} \in \binom{\N}{2}.\]
    %
...
\begin{example}[d-Uniform Hypergraph Chain]
    ...
    \noindent Corollary~\ref{cor:classification_binary_array_processes} below shows these are essentially the only binary exchangeable array processes.
\end{example}
...
encoded as a $d$-dimensional array whose entries encode these relationships. Exchangeability then enforces the
...
\]
where $y|_{[m]}$ restricts the finite array $y$ to entries $y_e$ with $e \subseteq [m]$. 
...
%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Boundary of the Unlabeled Chain $M_n$}
...
%
\[\P(L^h_0 = x_0, \ldots, L^h_n = x_n) = h(x_n) \P(L_0 = x_0, \ldots, L_n = x_n).\]
%
\noindent The right hand side is zero if the states $x_0, \ldots , x_n$ are not nested in the 
sense of arrays, $x_m = (x_n) |_{[m]}$ for $m < n$, and thus the same is true of the left 
hand side. Thus, by taking a projective limit, the chain $L_n^h$ a.s. defines an array ($X^h_e)$ of 
the same dimension as the original exchangeable array $(X_e)$ of which it is the corresponding array process.
...
%
\begin{align*}
    h(x_n)\P(L^h_0 = \tx_0, \ldots, L^h_n = \tx_n)  & = h(x_n)h(\tx_n) \P(L_0 = \tx_0, \ldots, L_n = \tx_n) \\
    %
    & = h(\tx_n)  h(x_n)\P(L_0 = x_0, \ldots, L_n = x_n) \\
    & = h(\tx_n)  \P(L^h_0 = x_0, \ldots, L^h_n = x_n).
\end{align*}

\noindent These two are equal if and only if $h(\tx_n) = h(x_n)$. Since $x, \sigma$ were arbitrary, the result 
follows from the fact that the finite dimensional distributions of our array 
$(X^h_e)$ are determined from the distribution of $(L_n^h)$.
\end{proof}
...
\begin{remark}
    ...
    %
        \[f(x_\emptyset, x_i,x_j,x_{ij}) := \ind \{x_{ij} \leq w(x_i,x_j)\},\]
    %
    \noindent of the associated binary exchangeable array process does not depend on $x_\emptyset$.
...
only subsets of $A$. Lastly, for a finite $d$-dimensional array indexed by $\binom{[k]}{d}$,
let $|x| := k$ denote its \emph{size}.
...
Suppose we have finite arrays $(x_e)$ and $(y_e)$ of size $n, n+m$ respectively.
...
\noindent Again suppose we have finite arrays $(x_e)$ and $(y_e)$ of size $n, n+m$.
...
in the graphon literature. Left-convergence is closely related to the Doob-Martin topology.
...
%
\noindent To see why, note when $d=2$ we have
%
...
resulting discrepancy is well-controlled.
...
theory, we show points of the minimal boundary can equivalently be viewed as
...
discuss a natural extension to hypergraphs and prove a corresponding hypergraph counting lemma. Lastly, we discuss some ways one might be able
