...
%
\noindent Since the morphism density and embedding density agree asymptotically by
Lemma~\ref{lem:relating_different_notions_of_embedding} (iv), (v), it is natural to also 
equate these for our representation functions:
%
    \[mor(x, f) := emb(x, f), \quad mor^W(x,f) := emb^W(x,f).\]
%
...
$f$ is the representing function of an exchangeable array $X$, then independent of $n$ we have
...
\end{equation}
%
\noindent Moreover, if $f$ is extremal (has no dependence on $x_\emptyset$), we have concentration
%
\begin{align*}
...
%
\begin{align*}
    mor^W(F, f_z) & := \int_{[0,1]^{2^k}} \prod_{S \in 1(F)} f_z(y_{(S)}) \; d\lambda_{2^k}(y) \\
    %
    & := \int_{[0,1]^{d}} \prod_{S \in 1(F)} \ind \left\{(\lceil |z| \cdot y_{s_1} \rceil,..., \lceil |z| \cdot y_{s_d} \rceil) \in edges(z) : S = \{s_i\}_{i=1}^d \right\} \; d\lambda_{d}(y) \\
    %
    & := \int_{[0,1]^{d}}  \ind \left\{ (i \mapsto \lceil |z| \cdot y_i \rceil)  \in mor^W(F,z)  \right\} \; d\lambda_{d}(y) \\[5pt]
    %
    & = mor^W(F,z) 
\end{align*}

\noindent In this way, the weak morphism densities are preserved, regardless of whether we view
...
theory, we show that the limit points of the given class of combinatorial objects can equivalently be thought of as either
the set of either as the set ergodic arrays, or as the set of corresponding representation functions
that depend trivially on the first coordinate $x_\emptyset$. 

We define several asymptotically equivalent notions of embedding one finite array into another. 
Using this, we show that this above probablistic viewpoint of the limit objects is equivalent to the 
more combinatorial notion of left-convergence built off these embedding densities, readily generalizing from the graphon literature.
...
This, for one, immediately shows that these topologies are compact, a feature that is far from obvious in the original
construction of~\cite{lovasz2004_limits_dense_graph_sequences}.

Our work so far omits one key success of the graphon theory: the cut-metric. For graphons, this metric metrizes the 
...
discuss a natural extension to hypergraphs, prove a corresponding hypergraph counting lemma with it, but then ultimately 
show that it is too fine to produce the topology of left-convergence. Furthermore, we discuss some ways one might be able
to extend any such metric from these hypergraphs (e.g. binary exchangealbe arrays) to exchangeable arrays with more general
...
\begin{proof}
 ...
 \; dx_{(S_m)}\right| \; (dx_e)_{e \nsubseteq S_m} \right]
    %
    \end{align*}
    \noindent The interior integral is upper bounded by our cut norm. After applying this bound, we can just integrate out the remaining factors, yielding the desired result.
\end{proof}

As in the case of graphons, the representation function is not unique, so 
...
based on compatible hierarchies of weak regularity partitions of lower-order faces. This notion admits subsequential
 limits and implies convergence of homomorphism densities. Since the Doob-Martin topology and topology of left-convergence
 are both compact, this is one step in the right direction. However, \cite{zhao2015_hypergraph_limits_regularity_approach}
...
First, we need to extend the notion of a weak embedding to the nonbinary case. For a subset of states
 ...
 implies $y_{\phi(e)} = s$. In the case $C = \{1\}$ and $S = \{0,1\}$ this reduces to the standard
  notion of weak embeddings of a binary array. Denote by $emb_C(x,y)$ the set of all such C-weak 
  embeddings and $emb^C(x,y)$ the corresponding embedding density. Then we have the following extension 
...
% proof formula
    ...
    %
    To see why the latter sets are disjoint, suppose that $\phi \in emb^{S \setminus \{0\}}(x, y_N) \cap emb^{S \setminus \{0\}}(x', y_N)$. 
    Note by the definition of $\cX^b_A$ that $x_e = 0$ iff $b_e=0$ iff $x_e' = 0$. Furthermore, for
     each $e$ where $x_e,x_e' \neq 0$ we have $x_e = (y_N)_{\phi(e)} = x'_e$. Thus $x = x'$.
...
% proof of converse
For the implication $(1) \to (2)$ suppose that we have an extremal bridge
 $M_n^{\alpha}$ associated to the minimal harmonic $\tK(\cdot, \alpha)$, 
 where $\alpha$ is in the minimal boundary  $\tE$. As mentioned in Appendix~\ref{appendix:doob_martin_background_appendix}, this is equivalent to the fact that this bridge has a trivial tail
$\sigma$-field almost surely. By Corollary~\ref{cor:unlabeled_associated_to_exchangeable_array} 
there is an exchangeable array $(X_e^\alpha)$ associated to this bridge. 
Suppose this array has distribution $\nu$ and that $I$ is a measurable 
$\nu$-invariant event. Let $r_n$ denote the function from infinite $d$-dimensional 
arrays to size $n$ arrays that just restricts the entries of $(X^\alpha_e)$ 
to $e \in [n]^{(d)}$. Then for every finite choice of $N \in \N$ because 
...
These proofs are extensions of the proofs in \cite{meliot2024_graphons_permutons_survey} of analogous 
results in the theory of graphons. Firstly (i) is immediate as any embedding is automatically 
a weak embedding. For (ii) note that if $\sigma \in Sym(\N)$ is such that $x \lesssim \sigma(y)$
 then there is precisely one $x' \geq x$ so that $x' \leq y$, namely $x'$ equals the array 
 $\sigma(y)$ restricted to $\{1,..., |x|\}^{(d)}$. For (iii) if for each $e$ where $x_e = 0$ we let $A_e$ be the set of weak embeddings of $x' = x + \delta_e$ into $y$ then:
%
...
of (ii) and (iii). For (i), exchangeability implies
...
%
\noindent where $z|_{[n]}$ is the restriction of $z$ to indices in $[n]^{(d)}$. Since any 
...
%
    \[= \frac{|\{z \in [y]_\sim : z\mid_{[n]} = x \}| }{|[y]_\sim|} \cdot |[x]_\sim| = \mathrm{emb}(x,y) \cdot |[x]_\sim|.\]
%
\noindent As a result, we have Doob-Martin kernel for the unlabeled chain
%
    \[K([x]_\sim, [y]_\sim) = \frac{\mathrm{emb}(x, y) |[x]_\sim|}{\P(M_n = [x]_\sim)} = \frac{\mathrm{emb}(x,y)}{\P(L_n = x)}.\]
%
...
\begin{equation*}
    emb(F,f) = \int_{[0,1]^{2^k}}  \prod_{S \in [k]^{(d)}} \ind \{f(x_{(S)}) = F_S\} \; d\lambda_{2^k}.
\end{equation*}
In particular, these quantities are independent of $n$. Moreover, if $f$ is extremal
\begin{align*}
...
$\phi : [k] \to [n]$ be injective. Then
    ...
    %
        \[f_{U_\emptyset}(u) := f(U_\emptyset, (u_i)_{i \in [d]}, ..., u_{[d]}).\]
    %
    \noindent If $f$ is extremal, it has no dependence on $U_\emptyset$ and thus $f_{U_\emptyset} = f$ is deterministic.
    ...
    The convergence then follows immediately from Borel-Cantelli. Lemma~\ref{lem:relating_different_notions_of_embedding} (iv) 
    and (v) allow the convergence to be extended to the morphism densities as well.
\end{proof}
