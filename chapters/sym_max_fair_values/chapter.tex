...
% continuization
We can re-state the above proof purely in terms of the quantile functions. Note if $X \sim \mu$ has 
quantile distribution $Q_X$, then the transformed random variable $Y = 2\theta X - c$ has 
quantile function $Q_Y(z) = 2\theta Q_X(z) - c$. Using Lemma~\ref{lem:quantile_fns_are_isometry}, then the objective in \eqref{eqn:imbalanced_optimization_problem} becomes:
%
...
barycenters and (ii) allows us to extend our fair value estimate from a point estimate
 ...
 For $p \neq 2$, we lose the linearity of the $p^{th}$ mean. However, we can still interpret $Z$ as follows: \red{todo}
...
\begin{proof}[Proof of Lemma~\ref{lem:fv_continuity_w2}]
    ...
    \zack{Well it seems like this suffices for the case $p=2$, but I am not sure what the general case looks like?}
\end{proof}
